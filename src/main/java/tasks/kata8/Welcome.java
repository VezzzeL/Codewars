...
// it in your database. It should default to English if the language is not in the database, or in the event
// of an invalid input.
public class Welcome {
    public static String greet(String language) {
        String data = "english:Welcome,czech:Vitejte,danish:Velkomst,dutch:Welkom,estonian:Tere tulemast," +
        ...
        Map<String, String>welcomeLang = Arrays.stream(data.split(","))
                .collect(Collectors.toMap(
                        k -> k.split(":")[0],
                        k -> k.split(":")[1]
                ));
        if (welcomeLang.containsKey(language)) {
            return welcomeLang.get(language);
        } else {
            return "Welcome";
        }
    }
